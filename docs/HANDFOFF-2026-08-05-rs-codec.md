...
| 机器 | IP | 角色 | 关键信息 |
|---|---|---|---|
| tokyo | <TOKYO_PUB_IP> (SSH 63916, <SSH_USER>) | 测试编码端 + 生产嵌套隧道 | 1vCPU 无 AVX-512; sudo 免密 |
| ali | <ALI_PUB_IP> (SSH 22, unkstar) | 测试解码端 + 生产汇聚 | 2C AVX-512; sudo 需密码 `<SUDO_PW>$` |
| osaka | <HOME_PUB_IP> (29832, unkstar) | 备用出口 | 与测试无关 |
...
# ali
echo '<SUDO_PW>$' | sudo -S systemctl restart great-hole-fec-test-ali

# 构建 (tokyo)
cd /home/<SSH_USER>/great-hole && git pull && cd obj2 && make -j2 great-hole
