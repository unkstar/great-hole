...
c = m:create_channel('<RS_PSK>', '127.0.0.1', 21001)
t = hole.tun('fec-a')
...
c = m:create_channel('<RS_PSK>', '127.0.0.1', 21000)
t = hole.tun('fec-b')
...
echo "$PW" | sudo -S nohup /home/<SSH_USER>/great-hole/build/src/great-hole --startlua /tmp/fec-a.lua > /tmp/fec-a.log 2>&1 &
echo "$PW" | sudo -S ip netns exec fecns nohup /home/<SSH_USER>/great-hole/build/src/great-hole --startlua /tmp/fec-b.lua > /tmp/fec-b.log 2>&1 &
