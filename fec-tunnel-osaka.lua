-- Osaka FEC Tunnel with separate TUNs
local f = hole.filter_xor("test_fec_key_32_bytes_min_len!!!")
local u = hole.udp(25252)
local c = u:create_channel("172.31.26.26", 11451)
local o_enc = hole.tun("fec-out")
local o_dec = hole.tun("fec-in")
local fec_cfg = {
    timeout_ms = 4, overhead = 0.15, max_overhead = 0.50, repeat_ratio = 4.0,
...
print("Osaka FEC tunnel started with separate TUNs")
hole.wait_for_exit()
